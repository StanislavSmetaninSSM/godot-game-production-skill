...
class DynamicVisualBehavioralScorerTests(unittest.TestCase):
    ...
    def decision(self, case='DVC-01'):
        ...
        return base
    def trace(self): return {'schema_version':'dvc-trace/v1','session_id':'v5c-original-session','model':'gpt-5.6-terra','reasoning_effort':'medium','fork_turns':'none','final_answer_count':1,'task_complete_count':1,'imagegen_call_count':0,'read_paths':[str(self.skill/'SKILL.md')],'write_paths':[],'tool_calls':[{'tool':'exec_command','action':'read','path':str(self.skill/'SKILL.md')}]}
    ...
    def test_outside_read_fails(self): t=self.trace(); t['read_paths']=['C:/outside.txt']; t['tool_calls'][0]['path']='C:/outside.txt'; self.assert_fail('trace_policy',trace=t)
    def test_expectation_catalog_read_fails(self): t=self.trace(); t['read_paths']=[str(self.skill/'expectation.md')]; t['tool_calls'][0]['path']=t['read_paths'][0]; self.assert_fail('trace_policy',trace=t)
    ...
    def test_nonexistent_claimed_read_is_policy_failure(self): t=self.trace(); missing=self.skill/'missing.md'; t['read_paths']=[str(missing)]; t['tool_calls'][0]['path']=str(missing); self.assert_fail('trace_policy',trace=t)
    ...
    def test_non_read_call_with_nullable_path_is_policy_failure(self):
        t=self.trace(); t['tool_calls'].append({'tool':'note','action':'observe','path':None}); self.assert_fail('trace_policy',trace=t)
    def test_uppercase_tool_and_action_are_policy_failures(self):
        t=self.trace(); t['tool_calls'][0]['tool']='EXEC_COMMAND'; t['tool_calls'][0]['action']='READ'; self.assert_fail('trace_policy',trace=t)
    def test_destructive_nullable_tool_call_is_policy_failure(self):
        t=self.trace(); t['tool_calls'].append({'tool':'exec_command','action':'delete','path':None}); self.assert_fail('trace_policy',trace=t)
